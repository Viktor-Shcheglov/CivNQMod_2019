...
-- Maori Starting Unit
-------------------------
local oldUnitType = "UNIT_SETTLER"
local oldunitID = GameInfo.Units.UNIT_SETTLER.ID
local newUnitOverride = GameInfo.Units.UNIT_MAORI_PIONEER.ID
local iCivType = GameInfo.Civilizations["CIVILIZATION_MC_MAORI"].ID
local Gameturn = Game.GetGameTurn()

function MaoriOverride(iPlayer, iUnit)
    local pPlayer = Players[iPlayer];
    if (pPlayer:IsEverAlive()) then
		if Gameturn <= 1 then
			if (pPlayer:GetCivilizationType() == iCivType) then
			...
			pUnit = pPlayer:GetUnitByID(iUnit);
					if (pUnit:GetUnitType() == oldunitID) then
						local newUnit = pPlayer:InitUnit(newUnitOverride, pUnit:GetX(), pUnit:GetY())
						newUnit:Convert(pUnit);
					end
				end
            end
...
Events.SerialEventUnitCreated.Add(MaoriOverride)

-- Tonga Starting Unit
-------------------------
local oldUnitType = "UNIT_WARRIOR"
local oldunitID = GameInfo.Units.UNIT_WARRIOR.ID
local newUnitOverride = GameInfo.Units.UNIT_MC_RAPA_NUI_MATATOA.ID
local iCivType = GameInfo.Civilizations["CIVILIZATION_MC_TONGA"].ID
local Gameturn = Game.GetGameTurn()
