...
function KilwaTrait(playerID)
    ...
    if player:IsEverAlive() and player:GetCivilizationType() == civilisationID then 
		for city in player:Cities() do
			city:SetNumRealBuilding(buildingTraitKilwaID, math.min(GetTradeRoutesNumber(player, city), 5)) -- I wonder if this will work
		end
	end
end
...
local iUkraineLeader = GameInfoTypes["LEADER_YAROSLAV"] -- More complex code would allow you to find all Leaders with Ukraine's Trait, but that's not really important
...
function Ukraine_CityFounded(playerID, iX, iY)
    ...
    if pPlayer:GetLeaderType() == iUkraineLeader then
        ...
        if pTeam:IsHasTech(iWheel) then -- Check whether the Team has the relevant Tech, rather than check for your other function having placed Buildings in existing Cities (as this would introduce logic errors)
